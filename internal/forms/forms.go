...
// // IsJson checks for a valid JSON input
// func (f *Form) IsJson(field string) {
// 	if !govalidator.IsJSON(f.Get(field)) {
// 		f.Errors.Add(field, "Invalid JSON")
// 	}
// }

// Required checks for required fields
func (f *Form) IsJson(fields ...string) {
	for _, field := range fields {
		value := f.Get(field)
